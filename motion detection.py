...
def motion_detection():
    ...
    while True:
        ...
        greyscale_frame = cv2.cvtColor(frame,
                                       cv2.COLOR_BGR2GRAY)  # make each frame greyscale which is needed for threshold
        ...
        blur_frame = cv2.blur(gaussian_frame, (
        5, 5))  # uses a kernal of size(5,5)(width,height) wich goes over 5x5 pixel area left to right
        ...
        # greyscale image with blur etc which is the final image ready to be used for threshold and motion detecion
        ...
        for c in cnt:
            if cv2.contourArea(c) > 800:  # if contour area is less then 800 non-zero(not-black) pixels(white)
                (x, y, w, h) = cv2.boundingRect(
                    c)  # x,y are the top left of the contour and w,h are the width and hieght
        ...
        key = cv2.waitKey(
            1) & 0xFF  # (1) = time delay in seconds before execution, and 0xFF takes the last 8 bit to check value or sumin
        if key == ord('q'):
